...
public:
    ...
    void testBlockAssignment() {
        ...
        TS_ASSERT_EQUALS(matrix(1, 1), 1);
        TS_ASSERT_EQUALS(matrix(2, 1), 1);
        TS_ASSERT_EQUALS(matrix(0, 2), 0);
        TS_ASSERT_EQUALS(matrix(1, 2), 1);
        TS_ASSERT_EQUALS(matrix(2, 2), 1);
    }

    void testDynamicBlockAssignment() {
        MatrixXxX matrix(10, 10);
        matrix.block(2, 3, 4, 5).setOnes();
        for (int colIndex = 0; colIndex < 10; ++colIndex) {
            for (int rowIndex = 0; rowIndex < 10; ++rowIndex) {
                if (rowIndex >= 2 && rowIndex < 6 && colIndex >= 3 && colIndex < 8) {
                    TS_ASSERT_EQUALS(matrix(rowIndex, colIndex), 1);
                } else {
                    TS_ASSERT_EQUALS(matrix(rowIndex, colIndex), 0);
                }
            }
        }
    }

    void testBlockToBlockAssignment() {
        Matrix3x3 matrix;

    ...
    namespace detail
    {
        ...
        ExpressionImplementation::deduplicated(DeduplicationCache& deduplicationCache) const {
            ...
            auto iterator = std::find_if(
                ...
                [this] (
                    const ExpressionImplementationPtr& cachedExpressionImplementation
                ) -> bool {
        ...
        ExpressionImplementationPtr
        ExpressionImplementation::composed(
            const ExpressionImplementationPtr& innerExpression
        ) const {
        ...
        ExpressionImplementation::dot(const ExpressionImplementationPtr& other) const {
            ...
            if (this->isConstantExpression() && other->isConstantExpression()) {
                ColumnMatrixXd thisColumnMatrix = 
                    this->cast<ConstantExpression>()->columnMatrix();
        ...
        operator*(
            ...
            // Determine which operand to use as the multiplier and which to use as the 
            // multiplicand: The multiplier should be a scalar (one dimensional), and if 
            // possible should be constant(to allow construction of a ScalingExpression 
            // instead of a generic ProductExpression).
            ...
            if (firstOperand->numDimensions() == 1 && secondOperand->numDimensions() == 1) {
                // Either the first or second argument could be the multiplier, so pick whichever
                // is constant (defaulting to the first argument)
                if (firstOperand->isConstantExpression()) {
